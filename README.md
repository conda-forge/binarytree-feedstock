...
Once the `conda-forge` channel has been enabled, `binarytree` can be installed with `conda`:
...
```
mamba install binarytree
```

It is possible to list all of the versions of `binarytree` available on your platform with `conda`:
...
```
mamba search binarytree --channel conda-forge
```
...
mamba repoquery search binarytree --channel conda-forge

# List packages depending on `binarytree`:
mamba repoquery whoneeds binarytree --channel conda-forge

# List dependencies of `binarytree`:
mamba repoquery depends binarytree --channel conda-forge
```
